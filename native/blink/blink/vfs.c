...
int VfsInit(const char *prefix) {
  ...
  if (VfsMkdir(AT_FDCWD, VFS_SYSTEM_ROOT_MOUNT, 0755) == -1 &&
      errno != EEXIST) {
#ifdef __wasm32__
    // WASI: root is not writable, but Blink VFS layer handles this internally
    LOGF("VFS: mkdir %s failed (expected on WASI), continuing", VFS_SYSTEM_ROOT_MOUNT);
#else
...
#ifdef __wasm32__
    LOGF("VFS: mkdir /dev failed (expected on WASI), continuing");
#else
...
#ifdef __wasm32__
    LOGF("VFS: mkdir /proc failed (expected on WASI), continuing");
#else
