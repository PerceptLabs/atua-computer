...
static int Exec(char *execfn, char *prog, char **argv, char **envp) {
  ...
  m->system->exec = Exec;
#ifdef __wasm32__
  // On WASI, prepend BLINK_PREFIX to program path for LoadProgram
  {
    const char *_pfx = getenv("BLINK_PREFIX");
    if (_pfx && prog[0] == '/' && strncmp(prog, _pfx, strlen(_pfx)) != 0) {
      size_t _plen = strlen(_pfx);
      size_t _slen = strlen(prog);
      char *_pp = (char *)malloc(_plen + _slen + 1);
      memcpy(_pp, _pfx, _plen);
      memcpy(_pp + _plen, prog, _slen + 1);
      prog = _pp;
      _slen = strlen(execfn);
      char *_ep = (char *)malloc(_plen + _slen + 1);
      memcpy(_ep, _pfx, _plen);
      memcpy(_ep + _plen, execfn, _slen + 1);
      execfn = _ep;
    }
  }
#endif
...
#ifdef __ATUA_BROWSER__
  /* In browser, there's no log file — route all log output to stderr
     (which goes to atua_term_write → browser console). */
  if (FLAG_strace) FLAG_alsologtostderr = true;
#endif
#endif
}
...
int main(int argc, char *argv[]) {
  ...
  if (FLAG_restore_fork) {
    ...
    extern void ResetTlb(struct Machine *);
    ResetTlb(fm);
    memset(fm->opcache, 0, sizeof(*fm->opcache));
    free(fbuf);
    fm->system->exec = Exec;
    fm->system->pid = getpid();
    fm->tid = getpid();
    fm->system->loaded = true;
    Put64(fm->ax, 0);
    fm->canhalt = true;
    fm->attention = false;
    fm->signals = 0;
...
#endif
#ifdef __wasm32__
  { extern void InitPagePool(size_t); InitPagePool(2048); }
#endif
  if (optind_ == argc) {
    PrintUsage(argc, argv, 48, 2);
  }
