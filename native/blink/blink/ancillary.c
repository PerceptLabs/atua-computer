...
#ifdef __wasm32__
/* WASI has no CMSG_DATA/SCM_RIGHTS — no Unix domain sockets.
   Provide the exported functions with no-op implementations. */
#include "blink/machine.h"
#include "blink/linux.h"
int SendAncillary(struct Machine *m, struct msghdr *msg,
                  const struct msghdr_linux *gm) {
  (void)m; (void)msg; (void)gm;
  return 0; /* no ancillary data to send */
}
int ReceiveAncillary(struct Machine *m, struct msghdr_linux *gm,
                     struct msghdr *msg, int flags) {
  (void)m; (void)gm; (void)msg; (void)flags;
  return 0; /* no ancillary data received */
}
#else /* !__wasm32__ */
