...
int SysPipe2(struct Machine *m, i64 pipefds_addr, i32 flags) {
#ifdef __ATUA_BROWSER__
  // Browser: create pipe via JS (returns pipe_id, fds are pipe_id*2, pipe_id*2+1)
  int oflags = 0;
  if (flags & O_CLOEXEC_LINUX) oflags |= O_CLOEXEC;
  if (flags & O_NDELAY_LINUX) oflags |= O_NDELAY;
  u8 fds_linux[2][4];
  int pipe_id = atua_pipe_create();
  if (pipe_id < 0) return -1;
  int read_fd = 200 + pipe_id * 2;
  int write_fd = 200 + pipe_id * 2 + 1;
  ...
  Write32(fds_linux[0], read_fd);
  Write32(fds_linux[1], write_fd);
