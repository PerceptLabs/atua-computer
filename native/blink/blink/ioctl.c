...
#ifdef __ATUA_BROWSER__
  /* Query terminal size from JS (xterm.js or default 80x24). */
  int rows = 24, cols = 80;
  atua_term_get_size(&rows, &cols);
  struct winsize_linux gws;
  Write16(gws.row, rows);
  Write16(gws.col, cols);
  Write16(gws.xpixel, cols * 8);
  Write16(gws.ypixel, rows * 16);
  return CopyToUserWrite(m, addr, &gws, sizeof(gws)) == -1 ? -1 : 0;
#else
  int rc;
  struct winsize ws;
  struct winsize_linux gws;
...
#ifdef __ATUA_BROWSER__
  /* Return a default termios matching a cooked-mode terminal.
     This is what bash/readline see on a fresh PTY — ICANON, ECHO on,
     OPOST|ONLCR on, CS8. Programs that want raw mode will TCSETS after. */
  struct termios_linux gtio;
  __builtin_memset(&gtio, 0, sizeof(gtio));
  Write32(gtio.iflag, ICRNL_LINUX | IXON_LINUX);
  Write32(gtio.oflag, OPOST_LINUX | ONLCR_LINUX);
  Write32(gtio.cflag, CS8_LINUX | CREAD_LINUX | B38400_LINUX);
  Write32(gtio.lflag, ISIG_LINUX | ICANON_LINUX | ECHO_LINUX |
                       ECHOE_LINUX | ECHOK_LINUX | IEXTEN_LINUX);
  gtio.cc[VINTR_LINUX] = 3;    /* ^C */
  gtio.cc[VQUIT_LINUX] = 28;   /* ^\ */
  gtio.cc[VERASE_LINUX] = 127; /* DEL */
  gtio.cc[VKILL_LINUX] = 21;   /* ^U */
  gtio.cc[VEOF_LINUX] = 4;     /* ^D */
  gtio.cc[VMIN_LINUX] = 1;
  gtio.cc[VSTART_LINUX] = 17;  /* ^Q */
  gtio.cc[VSTOP_LINUX] = 19;   /* ^S */
  gtio.cc[VSUSP_LINUX] = 26;   /* ^Z */
  return CopyToUserWrite(m, addr, &gtio, sizeof(gtio)) == -1 ? -1 : 0;
#else
  int rc;
  struct termios tio;
  struct termios_linux gtio;
...
static int IoctlTiocgpgrp(struct Machine *m, int fd, i64 addr) {
  ...
  // Force shells to disable job control in emscripten/wasi
  errno = ENOTTY;
  return -1;
#endif
