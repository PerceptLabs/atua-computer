...
int SysClose(struct Machine *m, i32 fildes) {
  ...
  int atua_close_type = -1;
  int atua_close_handle = -1;
#endif
  LOCK(&m->system->fds.lock);
  if ((fd = GetFd(&m->system->fds, fildes))) {
#ifdef __ATUA_BROWSER__
    /* Capture the routing info while we still hold the lock and the fd
       is alive. We'll call the JS import after releasing the lock. */
    if (fd->atua_type == ATUA_FD_PIPE || fd->atua_type == ATUA_FD_SOCKET) {
      atua_close_type = fd->atua_type;
      atua_close_handle = fd->atua_host_handle;
    }
#endif
  ...
  if (!fd) return -1;
#ifdef __ATUA_BROWSER__
  if (atua_close_type == ATUA_FD_PIPE && atua_close_handle >= 0) {
    __attribute__((import_module("atua"), import_name("pipe_close")))
    extern void atua_pipe_close(int pipe_id, int end);
    /* Pipe host handles encode pipe_id and end: handle = pipe_id * 2 + end.
       Offset by 200 to avoid collision with low fd numbers. */
    atua_pipe_close((atua_close_handle - 200) / 2,
                    (atua_close_handle - 200) % 2);
  }
  if (atua_close_type == ATUA_FD_SOCKET && atua_close_handle >= 0) {
    __attribute__((import_module("atua"), import_name("socket_close")))
    extern void atua_socket_close(int sock_id);
    atua_socket_close(atua_close_handle);
  }
#endif
