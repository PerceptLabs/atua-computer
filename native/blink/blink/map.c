...
#include "blink/map.h"
#ifdef __wasm32__
extern u8 *AllocPoolPages(size_t count);
extern int IsPoolPage(u8 *ptr);
#endif
...
static long GetSystemPageSize(void) {
  ...
  // "pages" in Emscripten/WASI only refer to the granularity the memory
...
void *Mmap(void *addr,     //
  ...
  {
    // WASI: use pool allocator for page-aligned allocations (no real mmap)
    size_t al = (length + 4095) & ~(size_t)4095;
    if (al < 4096) al = 4096;
    if (fd == -1 || (flags & MAP_ANONYMOUS_)) {
      res = AllocPoolPages(al / 4096);
      memset(res, 0, al);
    } else {
      res = AllocPoolPages(al / 4096);
      memset(res, 0, al);
      {
        ...
        while ((size_t)total < length) {
          ssize_t nr = pread(fd, (char *)res + total, length - total, offset + total);
          if (nr <= 0) break;
          total += nr;
        }
        // Short read OK: buffer was zero-filled by memset
      }
    }
    return res;
  }
...
#if defined(__wasm32__)
  if (IsPoolPage((u8 *)addr)) return 0;
  {
    size_t i;
