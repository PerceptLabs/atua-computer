...
#ifdef __wasm32__
/* Contiguous guest physical memory pool for WASI mmap replacement. */
struct PagePool {
    u8 *base;
    size_t capacity;
    size_t next_page;
};
struct PagePool g_pagepool;

void InitPagePool(size_t num_pages) {
    g_pagepool.capacity = num_pages * 4096;
    g_pagepool.base = (u8 *)aligned_alloc(4096, g_pagepool.capacity);
    if (!g_pagepool.base) { write(2, "pool alloc fail\n", 16); _exit(1); }
    memset(g_pagepool.base, 0, g_pagepool.capacity);
    g_pagepool.next_page = 0;
}

void GrowPagePool(size_t new_num_pages) {
    size_t new_cap = new_num_pages * 4096;
    u8 *new_base = (u8 *)aligned_alloc(4096, new_cap);
    if (!new_base) { write(2, "pool grow fail\n", 15); _exit(1); }
    memset(new_base, 0, new_cap);
    memcpy(new_base, g_pagepool.base, g_pagepool.next_page * 4096);
    u8 *old_base = g_pagepool.base;
    size_t old_cap = g_pagepool.capacity;
    size_t i;
    /* Fix up host page pointers into new allocation. */
    for (i = 0; i < g_hostpages.n; i++) {
        ptrdiff_t off = g_hostpages.p[i] - old_base;
        if (off >= 0 && (size_t)off < old_cap)
            g_hostpages.p[i] = new_base + off;
    }
    /* Fix up free page list pointers too. */
    struct HostPage *h;
    LOCK(&g_allocator.lock);
    for (h = g_allocator.pages; h; h = h->next) {
        ptrdiff_t off = h->page - old_base;
        if (off >= 0 && (size_t)off < old_cap)
            h->page = new_base + off;
    }
    UNLOCK(&g_allocator.lock);
    free(old_base);
    g_pagepool.base = new_base;
    g_pagepool.capacity = new_cap;
}

u8 *AllocPoolPage(void) {
    if (g_pagepool.next_page * 4096 >= g_pagepool.capacity)
        GrowPagePool(g_pagepool.capacity / 4096 * 2);
    u8 *page = g_pagepool.base + (g_pagepool.next_page * 4096);
    g_pagepool.next_page++;
    return page;
}

u8 *AllocPoolPages(size_t count) {
    while ((g_pagepool.next_page + count) * 4096 > g_pagepool.capacity)
        GrowPagePool(g_pagepool.capacity / 4096 * 2);
    u8 *base = g_pagepool.base + (g_pagepool.next_page * 4096);
    g_pagepool.next_page += count;
    return base;
}

int IsPoolPage(u8 *ptr) {
