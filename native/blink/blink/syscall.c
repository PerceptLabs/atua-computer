...
extern u8 *AllocPoolPages(size_t);

/* ── Fork state serialization ───────────────────────────────────────────
 *
 * Under WASI there is no fork(). Instead we serialize the entire machine
 * state to a byte buffer, then either:
 *   - __ATUA_BROWSER__: send the buffer to JS which spawns a Web Worker
 *   - plain __wasm32__: write to a file and posix_spawn a child process
 *
 * The child calls DeserializeForkState to restore the machine. */
...
uint8_t *SerializeForkState(struct Machine *m, size_t *out_len) {
  ...
  fork_buf_write(m->beg, 128);
  fork_buf_write(m->xmm, sizeof(m->xmm));
  W(m->ip); W(m->flags);
  fork_buf_write(m->seg, sizeof(m->seg));
  W(m->fs); W(m->gs);
...
uint8_t *ReadForkFile(const char *path, size_t *out_len) {
  ...
  *out_len = st.st_size;
  ...
  size_t total = 0;
  while (total < *out_len) {
    ssize_t nr = read(fd, buf + total, *out_len - total);
    if (nr <= 0) break;
    total += nr;
  }
  close(fd);
...
static int SysFork(struct Machine *m) {
  ...
  { static int fork_counter = 0;
    snprintf(path, sizeof(path), "/tmp/blink-fork-%d", fork_counter++); }
  if (WriteForkFile(path, buf, len) < 0) { free(buf); return -1; }
  free(buf);
  extern char *g_blink_path;
  char *argv[] = { g_blink_path, "--restore-fork", (char *)path, NULL };
  pid_t child;
  if (posix_spawn(&child, g_blink_path, NULL, NULL, argv, environ) != 0)
    return -1;
  return child;
#else
  return Fork(m, 0, 0, 0);
...
#if defined(__ATUA_BROWSER__) || defined(__wasm32__)
    return SysFork(m);
...
static int SysSocket(struct Machine *m, i32 family, i32 type, i32 protocol) {
...
#ifdef __ATUA_BROWSER__
  {
    ...
    int oflags = O_RDWR;
    if (flags & SOCK_CLOEXEC_LINUX) oflags |= O_CLOEXEC;
    if (flags & SOCK_NONBLOCK_LINUX) oflags |= O_NDELAY;
    ...
    if (fd) {
      ...
      fd->socktype = raw_type == SOCK_STREAM ? SOCK_STREAM
                   : raw_type == SOCK_DGRAM  ? SOCK_DGRAM
                   : raw_type;
    }
    UNLOCK(&m->system->fds.lock);
    return sock_id;
...
static int SysGetsockname(struct Machine *m, int fd, i64 aa, i64 asa) {
#ifdef __ATUA_BROWSER__
  {
    ...
    if (fde && fde->atua_type == ATUA_FD_SOCKET) {
      /* Return synthetic 0.0.0.0:49153 — the browser has no real local address */
      u8 addr[16] = {0};
      addr[0] = 2; /* AF_INET */
      addr[2] = 0xC0; addr[3] = 0x01; /* port 49153 big-endian */
      u8 size_buf[4];
      Write32(size_buf, 16);
      if (aa) CopyToUserWrite(m, aa, addr, 16);
      if (asa) CopyToUserWrite(m, asa, size_buf, 4);
      UNLOCK(&m->system->fds.lock);
      return 0;
    }
    UNLOCK(&m->system->fds.lock);
...
static i64 SysSendto(struct Machine *m,  //
...
#ifdef __ATUA_BROWSER__
  {
    ...
    if (fd_type == ATUA_FD_SOCKET) {
      ...
      return atua_socket_send(fd_handle, buf, (int)buflen);
    }
  }
#endif
  ssize_t rc;
...
static i64 SysRecvfrom(struct Machine *m,  //
...
#ifdef __ATUA_BROWSER__
  {
    ...
    if (fd_type == ATUA_FD_SOCKET) {
      if (buflen > NUMERIC_MAX(size_t)) return eoverflow();
      if (buflen == 0) return 0;
      /* Read in 4KB chunks via temp buffer to handle page-spanning guest
         buffers safely (SchlepW may return a malloc copy that gets freed). */
      i64 total = 0;
      int last_n = 0;
      while (total < (i64)buflen) {
        u8 tmp[4096];
        int chunk = (buflen - total) < 4096 ? (int)(buflen - total) : 4096;
        last_n = atua_socket_recv(fd_handle, tmp, chunk);
        if (last_n <= 0) break;
        if (CopyToUserWrite(m, bufaddr + total, tmp, last_n) == -1) return -1;
        total += last_n;
        if (last_n < chunk) break;
      }
      /* Key lesson: return eagain() not 0 when recv returns -1 with no
         prior data. Returning 0 means EOF; musl's DNS resolver needs
         EAGAIN to exit its recvfrom loop on non-blocking sockets. */
      if (total > 0) return total;
      return last_n < 0 ? eagain() : 0;
    }
  }
#endif
  ssize_t rc;
...
static i64 SysSendmsg(struct Machine *m, i32 fildes, i64 msgaddr, i32 flags) {
#ifdef __ATUA_BROWSER__
  {
    ...
    UNLOCK(&m->system->fds.lock);
    if (fd_type == ATUA_FD_SOCKET) {
      /* Flatten iovecs and send in chunks */
      const struct msghdr_linux *gm;
      if (!(gm = (const struct msghdr_linux *)SchlepR(m, msgaddr, sizeof(*gm)))) return -1;
      i64 iovaddr = Read64(gm->iov);
      u64 iovlen = Read64(gm->iovlen);
      i64 total = 0;
      for (u64 j = 0; j < iovlen; j++) {
        const struct iovec_linux *giov;
        if (!(giov = (const struct iovec_linux *)SchlepR(m, iovaddr + j * sizeof(*giov), sizeof(*giov)))) return -1;
        i64 base = Read64(giov->base);
        u64 iolen = Read64(giov->len);
        if (iolen == 0) continue;
        u8 *buf;
        if (!(buf = (u8 *)SchlepR(m, base, iolen))) return -1;
        int n = atua_socket_send(fd_handle, buf, (int)iolen);
        if (n <= 0) break;
        total += n;
      }
      return total;
...
static i64 SysRecvmsg(struct Machine *m, i32 fildes, i64 msgaddr, i32 flags) {
#ifdef __ATUA_BROWSER__
  {
    ...
    if (fd_type == ATUA_FD_SOCKET) {
      const struct msghdr_linux *gm;
      if (!(gm = (const struct msghdr_linux *)SchlepR(m, msgaddr, sizeof(*gm)))) return -1;
      i64 iovaddr2 = Read64(gm->iov);
      u64 iovlen2 = Read64(gm->iovlen);
      i64 total = 0;
      int last_n = 0;
      for (u64 j = 0; j < iovlen2 && total == 0; j++) {
        const struct iovec_linux *giov;
        if (!(giov = (const struct iovec_linux *)SchlepR(m, iovaddr2 + j * sizeof(*giov), sizeof(*giov)))) return -1;
        i64 base = Read64(giov->base);
        u64 iolen = Read64(giov->len);
        if (iolen == 0) continue;
        u8 tmp[4096];
        int chunk = iolen < 4096 ? (int)iolen : 4096;
        last_n = atua_socket_recv(fd_handle, tmp, chunk);
        if (last_n <= 0) break;
        if (CopyToUserWrite(m, base, tmp, last_n) == -1) return -1;
        total += last_n;
      }
      if (total > 0) return total;
      return last_n < 0 ? eagain() : 0;
...
static int SysBind(struct Machine *m, int fd, i64 aa, u32 as) {
#ifdef __ATUA_BROWSER__
  {
    ...
    LOCK(&m->system->fds.lock);
    fde = GetFd(&m->system->fds, fd);
    if (fde) fd_type = fde->atua_type;
    UNLOCK(&m->system->fds.lock);
    if (fd_type == ATUA_FD_SOCKET) return 0; /* no-op — browser sockets don't bind */
  }
#endif
...
static int SysConnect(struct Machine *m, int fd, i64 aa, u32 as) {
#ifdef __ATUA_BROWSER__
  {
    ...
    UNLOCK(&m->system->fds.lock);
    if (fd_type == ATUA_FD_SOCKET) {
      /* Copy the guest sockaddr_in so JS can read the IP and port */
      u8 addr_buf[128];
...
static int SysSetsockopt(struct Machine *m, i32 fildes, i32 level, i32 optname,
...
#ifdef __ATUA_BROWSER__
  {
    ...
    UNLOCK(&m->system->fds.lock);
    if (fd_type == ATUA_FD_SOCKET) return 0; /* no-op — browser ignores socket options */
  }
#endif
  int rc;
  void *optval;
...
static int SysGetsockopt(struct Machine *m, i32 fildes, i32 level, i32 optname,
...
#ifdef __ATUA_BROWSER__
  {
    ...
    UNLOCK(&m->system->fds.lock);
    if (fd_type == ATUA_FD_SOCKET) {
      /* Return sensible defaults for common options */
      u8 optvalsize_buf[4];
      ...
      if (optvalsize >= 4) {
        ...
        /* SO_ERROR → 0 (no error) is the default from memset */
...
static i64 SysRead(struct Machine *m, i32 fildes, i64 addr, u64 size) {
#ifdef __ATUA_BROWSER__
  {
    ...
    struct Fd *fde;
    LOCK(&m->system->fds.lock);
    fde = GetFd(&m->system->fds, fildes);
    if (fde) { fd_type = fde->atua_type; fd_handle = fde->atua_host_handle; }
    UNLOCK(&m->system->fds.lock);
    if (fd_type == ATUA_FD_STDIN) {
      if (size > NUMERIC_MAX(size_t)) return eoverflow();
      /* Use temp buffer + CopyToUserWrite to handle page-spanning
         guest buffers safely (SchlepW may return a disposable copy). */
      u8 tmp[4096];
      int chunk = size < 4096 ? (int)size : 4096;
      int n = atua_term_read(tmp, chunk);
      if (n > 0) CopyToUserWrite(m, addr, tmp, n);
      return n;
    }
    if (fd_type == ATUA_FD_PIPE) {
      ...
      u8 tmp[4096];
      int chunk = size < 4096 ? (int)size : 4096;
      int n = atua_pipe_read(pipe_id, tmp, chunk);
      if (n > 0) CopyToUserWrite(m, addr, tmp, n);
      return n >= 0 ? n : -1;
    }
    if (fd_type == ATUA_FD_SOCKET) {
      ...
      i64 total = 0;
      int last_n = 0;
      while (total < (i64)size) {
        u8 tmp[4096];
        int chunk = (size - total) < 4096 ? (int)(size - total) : 4096;
        last_n = atua_socket_recv(fd_handle, tmp, chunk);
        if (last_n <= 0) break;
        if (CopyToUserWrite(m, addr + total, tmp, last_n) == -1) return -1;
        total += last_n;
        if (last_n < chunk) break;
      }
      if (total > 0) return total;
...
static i64 SysWrite(struct Machine *m, i32 fildes, i64 addr, u64 size) {
#ifdef __ATUA_BROWSER__
  {
    ...
    if (fde) { fd_type = fde->atua_type; fd_handle = fde->atua_host_handle; }
    UNLOCK(&m->system->fds.lock);
    if (fd_type == ATUA_FD_STDOUT || fd_type == ATUA_FD_STDERR) {
      u8 *buf;
      if (size > NUMERIC_MAX(size_t)) return eoverflow();
      if (!(buf = (u8 *)SchlepR(m, addr, size))) return -1;
      atua_term_write(buf, (int)size);
      return size;
    }
    if (fd_type == ATUA_FD_PIPE) {
      int pipe_id = (fd_handle - 200) / 2;
...
#ifdef __ATUA_BROWSER__
  {
    /* Stdin reads come through preadv/readv (bash uses readv on fd 0).
       Route to atua_term_read for the first iovec buffer. */
    int fd_type = -1;
    struct Fd *fde;
    LOCK(&m->system->fds.lock);
    fde = GetFd(&m->system->fds, fildes);
    if (fde) fd_type = fde->atua_type;
    UNLOCK(&m->system->fds.lock);
    if (fd_type == ATUA_FD_STDIN && iovlen > 0) {
      const struct iovec_linux *giov;
      if (!(giov = (const struct iovec_linux *)SchlepR(
                m, iovaddr, sizeof(*giov)))) return -1;
      i64 base = Read64(giov->base);
      u64 len = Read64(giov->len);
      if (len > 0) {
        u8 tmp[4096];
        int chunk = len < 4096 ? (int)len : 4096;
        int n = atua_term_read(tmp, chunk);
        if (n > 0) CopyToUserWrite(m, base, tmp, n);
        return n;
      }
      return 0;
    }
    if (fd_type == ATUA_FD_PIPE) {
      ...
      int fd_handle = -1;
      LOCK(&m->system->fds.lock);
      fde2 = GetFd(&m->system->fds, fildes);
      if (fde2) fd_handle = fde2->atua_host_handle;
      UNLOCK(&m->system->fds.lock);
      if (fd_handle >= 0 && iovlen > 0) {
        int pipe_id = (fd_handle - 200) / 2;
        const struct iovec_linux *giov;
        if (!(giov = (const struct iovec_linux *)SchlepR(
                  m, iovaddr, sizeof(*giov)))) return -1;
        ...
        u64 len = Read64(giov->len);
        u8 *buf;
        if (!(buf = (u8 *)SchlepW(m, base, len))) return -1;
        return atua_pipe_read(pipe_id, buf, (int)len);
      }
    }
  }
#endif
...
#ifdef __ATUA_BROWSER__
  {
    /* Route writev for stdout/stderr/pipe/socket the same as write(). */
    int fd_type = -1, fd_handle = -1;
    struct Fd *fde;
    LOCK(&m->system->fds.lock);
    fde = GetFd(&m->system->fds, fildes);
    ...
    UNLOCK(&m->system->fds.lock);
    if ((fd_type == ATUA_FD_STDOUT || fd_type == ATUA_FD_STDERR ||
         fd_type == ATUA_FD_PIPE || fd_type == ATUA_FD_SOCKET) && iovlen > 0) {
      i64 total = 0;
      for (u32 j = 0; j < iovlen; j++) {
        const struct iovec_linux *giov;
        if (!(giov = (const struct iovec_linux *)SchlepR(
                  m, iovaddr + j * sizeof(*giov), sizeof(*giov)))) return -1;
        i64 base = Read64(giov->base);
        u64 len = Read64(giov->len);
        if (len == 0) continue;
        u8 *buf;
        if (!(buf = (u8 *)SchlepR(m, base, len))) return -1;
        if (fd_type == ATUA_FD_STDOUT || fd_type == ATUA_FD_STDERR) {
          atua_term_write(buf, (int)len);
          total += len;
        } else if (fd_type == ATUA_FD_PIPE) {
          int n = atua_pipe_write((fd_handle - 200) / 2, buf, (int)len);
          if (n > 0) total += n;
          if (n <= 0) break;
        } else if (fd_type == ATUA_FD_SOCKET) {
          int n = atua_socket_send(fd_handle, buf, (int)len);
          if (n > 0) total += n;
          if (n <= 0) break;
        }
      }
...
#ifdef __wasm32__
/* WASI sysroot defines S_IFIFO as 0xc000 which differs from Linux.
   Force POSIX values for the static asserts and mode translation. */
#undef S_IFIFO
...
#ifdef __ATUA_BROWSER__
  {
    /* Browser: wait for child Worker via SAB + Atomics.wait */
    int status = 0;
    int rc = atua_proc_wait(pid, &status);
    if (rc >= 0 && opt_out_wstatus_addr) {
      /* Encode exit code as Linux wstatus: (code << 8) for normal exit */
      u8 wstatusb[4];
      Write32(wstatusb, (status & 0xFF) << 8);
      CopyToUserWrite(m, opt_out_wstatus_addr, wstatusb, 4);
    }
    return rc;
  }
#endif
    ...
    TryAgain:
      ...
      if (fd) {
        ...
        if (fd->atua_type == ATUA_FD_SOCKET) {
          int sp = atua_socket_poll(fd->atua_host_handle);
          if (FD_ISSET(fildes, &readfds) && (sp & 1)) {
            ++rc; FD_SET(fildes, &readyreadfds); FD_CLR(fildes, &readfds);
          }
          if (FD_ISSET(fildes, &writefds) && (sp & 2)) {
            ++rc; FD_SET(fildes, &readywritefds); FD_CLR(fildes, &writefds);
          }
          if (sp & 4) {
            ++rc; FD_SET(fildes, &readyexceptfds); FD_CLR(fildes, &exceptfds);
          }
          continue;
        }
#endif
...
#ifdef __ATUA_BROWSER__
            if (fd->atua_type == ATUA_FD_SOCKET) {
              /* Ask JS if the socket has data/is writable/hung up */
              int sp = atua_socket_poll(fd->atua_host_handle);
              int avail = 0;
              if (sp & 1) avail |= POLLIN_LINUX;
              if (sp & 2) avail |= POLLOUT_LINUX;
              if (sp & 4) avail |= POLLHUP_LINUX;
              int matched = avail & (ev | POLLERR_LINUX | POLLHUP_LINUX);
              if (matched) {
                ++rc;
                Write16(gfds[i].revents, matched);
              } else {
