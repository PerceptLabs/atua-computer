...
void AddStdFd(struct Fds *fds, int fildes) {
  int flags;
#ifdef __ATUA_BROWSER__
  /* Under WASI in the browser there are no real host fds — VfsFcntl fails.
     We know what fds 0-2 are: stdin is terminal input, stdout/stderr are
     terminal output. Create them directly with the right routing type. */
  if (fildes <= 2) {
    struct Fd *fd = AddFd(fds, fildes, fildes == 0 ? O_RDONLY : O_WRONLY);
    if (fd) {
      fd->atua_type = fildes == 0 ? ATUA_FD_STDIN
                    : fildes == 1 ? ATUA_FD_STDOUT
                                  : ATUA_FD_STDERR;
    }
    InheritFd(fd);
    return;
