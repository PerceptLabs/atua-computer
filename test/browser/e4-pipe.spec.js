/**
 * E4 test: echo hello | cat works in browser.
 * Pipe between two fork children via SharedArrayBuffer.
 */
...
test('echo hello | cat produces output in browser', async ({ page }) => {
  ...
  try {
    await page.waitForFunction(() => {
      ...
      return text.includes('hello') || text.includes('ERROR:') || text.includes('exited');
    }, { timeout: 30000 });
  } catch (e) { /* timeout ok */ }

  const pageText = await page.textContent('#output');
  console.log('Console:', consoleMessages.slice(0, 10).join('\n'));
